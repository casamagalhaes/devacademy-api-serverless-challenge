...
module.exports = class CustomerService {
  constructor() {
    ...
    this._modelClass = Customer;
  }

  async validateBeforeSave(data) {
    if (!data.name) throw new ValidationError("name is required");
    if (!data.age) throw new ValidationError("age is required");
    if (data.age < 0 || data.age >= 125) throw new ValidationError("age must be greater than 0 and lower than 125");
  }
  ...
  async findById(id) {
    try {
      ...
      if (e.name === "ItemNotFoundException") throw new NotFoundError("customer not found");
      throw e;
    }
  }
